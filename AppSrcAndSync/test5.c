...
int main(int argc, char *argv[])
{
    ...
    g_print( "time starts from: %" G_GUINT64_FORMAT, clock_time);
    return;
	...
	pipeline = gst_parse_launch("udpsrc port=5001 ! rtpatimeparse ! application/x-rtp, media=(string)video, clock-rate=(int)90000, payload=(int)96 ! rtph264depay ! appsink name=mysink", NULL);
    
    gst_element_set_start_time(pipeline, 1);

    GstElement * appsink = gst_bin_get_by_name_recurse_up (GST_BIN (pipeline), "mysink");
    GstElement * jitbuf = gst_bin_get_by_name_recurse_up (GST_BIN (pipeline), "buf");
    
    if (jitbuf != NULL)
    {
        g_signal_connect (jitbuf, "handle-sync", G_CALLBACK (jitcb), NULL);
    } else {
        g_print("NULL!\n");
    }
